...
public class InitMcp23XInputOperation : OperationBase
{
    public const string OpCodeValue = "init_mcp23x_input";
    ...
    protected override ValueTask<OperationExecutionResult> ExecuteInternalAsync(IOperationExecutionContext context)
    {
        ...
        {
            context.Log(LogLevel.Error, $"The specified mcp variable '{mcpVariable.Value.VariableName}' does not exist");
            return new ValueTask<OperationExecutionResult>(OperationExecutionResult.Failure());
        }
        
        if (mcpObject is not Mcp23xxx mcp)
        {
            context.Log(LogLevel.Error, $"The specified mcp variable '{mcpVariable.Value.VariableName}' is not an MCP23008 instance");
            return new ValueTask<OperationExecutionResult>(OperationExecutionResult.Failure());
        }
        ...
        var inputPort = mcp.CreateDigitalInputPort(pin);
        context.Outputs[variable.Value.VariableName] = inputPort;
        
        context.Log(LogLevel.Debug,
            $"Digital input port created for MCP pin ${pinName} to variable '${variable.Value.VariableName}'");
